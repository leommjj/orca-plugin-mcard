...
function validateInboxSettings(settings) {
  return !!(settings.inboxApiUrl || settings.s3Endpoint && settings.s3AccessKey && settings.s3SecretKey && settings.s3Bucket);
}
async function fetchInboxApi(apiUrl) {
...
async function deleteInboxApiItem(apiBaseUrl, objectId) {
  const base = apiBaseUrl.replace(/\/+$/, "");
  const parentDir = base.substring(0, base.lastIndexOf("/") + 1);
  const deleteUrl = `${parentDir}${objectId}`;
...
function findBacklinkNotes(notes, targetNote) {
  const searchKey = targetNote.title || targetNote.id;
  if (!searchKey) return [];
  const regex = new RegExp(`\\[\\[${escapeRegex(searchKey)}\\]\\]`, "g");
  return notes.filter((n) => n.id !== targetNote.id && regex.test(n.content));
}
...
function McardPanel({ pluginName: pluginName2, settings, onSettingsChange, panelId }) {
  ...
  const [inboxApiSyncing, setInboxApiSyncing] = React.useState(false);
  const [inboxApiMessage, setInboxApiMessage] = React.useState("");
  ...
  const [memoInputMode, setMemoInputMode] = React.useState("child");
  ...
  const [detailFromExpr, setDetailFromExpr] = React.useState(false);
  const [detailNavStack, setDetailNavStack] = React.useState([]);
  const [toastMsg, setToastMsg] = React.useState("");
  const [toastTimer, setToastTimer] = React.useState(null);
  function showToast(msg) {
    setToastMsg(msg);
    if (toastTimer) clearTimeout(toastTimer);
    const t2 = setTimeout(() => setToastMsg(""), 3e3);
    setToastTimer(t2);
  }
  React.useEffect(() => {
    if (flomoMessage && !flomoSyncing) showToast(flomoMessage);
  }, [flomoMessage, flomoSyncing]);
  React.useEffect(() => {
    if (inboxMessage && !inboxSyncing) showToast(inboxMessage);
  }, [inboxMessage, inboxSyncing]);
  React.useEffect(() => {
    if (inboxApiMessage && !inboxApiSyncing) showToast(inboxApiMessage);
  }, [inboxApiMessage, inboxApiSyncing]);
  React.useEffect(() => {
    let cancelled = false;
    void (async () => {
  ...
  async function handleInboxApiSync() {
    if (!inboxConfig.inboxApiUrl) {
      setInboxApiMessage("请先配置 Inbox API URL");
      return;
    }
    setInboxApiSyncing(true);
    setInboxApiMessage("正在从 API 获取数据...");
    try {
      const items = await fetchInboxApi(inboxConfig.inboxApiUrl);
      if (items.length === 0) {
        setInboxApiMessage("没有新的 Inbox 数据");
        setInboxApiSyncing(false);
        return;
      }
      const existingIds = new Set(notes.map((n) => n.id));
      const newNotes = [];
      ...
      for (const item of items) {
        if (existingIds.has(item.objectId)) continue;
        newNotes.push({
          id: item.objectId,
          blockId: 0,
          content: item.content,
      ...
      if (added > 0) {
        ...
        for (const item of items) {
          try {
            await deleteInboxApiItem(inboxConfig.inboxApiUrl, item.objectId);
            deleted++;
          } catch (err) {
            console.warn("[inbox-api] 删除云端条目失败:", item.objectId, err);
          }
        }
        setInboxApiMessage(`同步完成，新增 ${added} 条，云端删除 ${deleted} 条`);
      }
    } catch (err) {
      setInboxApiMessage("API 同步失败：" + ((err == null ? void 0 : err.message) || String(err)));
      console.error("[inbox-api-sync] Error:", err);
    }
    setInboxApiSyncing(false);
  }
  ...
  const filteredNotes = React.useMemo(() => {
    ...
    if (!settings.showMemoNotesInList) {
      list = list.filter((n) => !n.memos || n.memos.length === 0);
    }
    if (selectedTag) {
      list = list.filter(
  ...
  function _renderMentionPicker(picker, searchText2, input, onInsert) {
    ...
    let matched = notes.filter((n) => !n.memos || n.memos.length === 0 || n.memos && n.memos.length > 0 && settings.showMemoNotesInList);
    if (kw) {
  ...
  function navigateToDetail(note) {
    if (detailNote) {
      setDetailNavStack((prev) => [...prev, detailNote]);
    }
    setDetailNote(note);
    setDetailEditing(false);
  }
  function goBackFromDetail() {
    if (detailNavStack.length > 0) {
      ...
      const fromExpr = detailFromExpr;
      setDetailNote(null);
      setDetailFromExpr(false);
      setDetailNavStack([]);
      if (fromExpr) setShowSearchExprModal(true);
    }
  }
    ...
    "div",
    {
      ...
      children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-shell", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-sidebar", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-sidebar-bottom", children: [
            flomoConfig.accessToken && /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "button",
              {
                className: "mcard-nav-item",
                onClick: () => void handleFlomoSync(),
                title: "Flomo 同步",
                disabled: flomoSyncing,
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "🌊" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "mcard-nav-tooltip", children: "Flomo同步" })
                ]
              }
            ),
            validateInboxSettings(inboxConfig) && /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "button",
              {
                className: "mcard-nav-item",
                onClick: () => void handleInboxSync(),
                title: "Inbox 同步",
                disabled: inboxSyncing,
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "📦" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "mcard-nav-tooltip", children: "Inbox同步" })
                ]
              }
            ),
            inboxConfig.inboxApiUrl && /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "button",
              {
                className: "mcard-nav-item",
                onClick: () => void handleInboxApiSync(),
                title: "Inbox API 同步",
                disabled: inboxApiSyncing,
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "🔌" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "mcard-nav-tooltip", children: "Inbox API同步" })
                ]
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  ...
                  "div",
                  {
                    ...
                    children: [
                      ...
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "mcard-menu-count", children: (() => {
                        let r = [...notes];
                        if (!settings.showMemoNotesInList) r = r.filter((n) => !n.memos || n.memos.length === 0);
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-main", children: [
                ...
                group.notes.map((note) => /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "div",
                  {
                    ...
                    children: editingNoteId === note.id ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-input-box focused mcard-inline-edit", onClick: (e) => e.stopPropagation(), children: [
                        ...
                        /* @__PURE__ */ jsxRuntimeExports.jsx(
                          "textarea",
                          {
                            ...
                            onKeyDown: (e) => {
                              handleEditKeyDown(e);
                              if (settings.enterToSubmit) {
                                if (e.key === "Enter" && !e.shiftKey && !showEditTagPicker) {
                                  e.preventDefault();
                              ...
                              if (e.key === "Escape" && !showEditTagPicker) {
                                e.preventDefault();
                                cancelEditNote();
                              }
                            },
                      ...
                      const childNotes = notes.filter((n) => n.parent_ids && n.parent_ids.includes(note.id));
                      const memoNotes = notes.filter((n) => n.memos && n.memos.includes(note.id));
                      ...
                      return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
                        ...
                        childNotes.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-note-relation", onClick: (e) => e.stopPropagation(), children: [
                          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "mcard-parent-icon", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: [
                            /* @__PURE__ */ jsxRuntimeExports.jsx("circle", { cx: "18", cy: "6", r: "3" }),
                            /* @__PURE__ */ jsxRuntimeExports.jsx("circle", { cx: "6", cy: "12", r: "3" }),
                            /* @__PURE__ */ jsxRuntimeExports.jsx("circle", { cx: "18", cy: "18", r: "3" }),
                        ...
                        memoNotes.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-note-relation", onClick: (e) => e.stopPropagation(), children: [
                          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "mcard-parent-icon", children: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", width: "14", height: "14", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { d: "M22 17a2 2 0 0 1-2 2H6.828a2 2 0 0 0-1.414.586l-2.202 2.202A.71.71 0 0 1 2 21.286V5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2z" }) }) }),
                        ...
                        (() => {
                          const backlinkNotes = findBacklinkNotes(notes, note);
                          if (backlinkNotes.length === 0) return null;
                          return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-note-relation", onClick: (e) => e.stopPropagation(), children: [
                            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "mcard-parent-icon", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: [
                            ...
                            backlinkNotes.map((blNote) => {
                              const preview = getNotePreview(blNote);
                              return /* @__PURE__ */ jsxRuntimeExports.jsx(
                                "div",
                                {
                                  className: "mcard-note-relation-item",
                        ...
                        isAddingMemo && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-input-box focused", style: { marginTop: "6px" }, onClick: (e) => e.stopPropagation(), children: [
                          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mcard-input-wrapper", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
                            "textarea",
                            {
                              ...
                              onKeyDown: (e) => {
                                const submitMemo = async () => {
                                  ...
                                  const newNote = {
                                    ...
                                    ...memoInputMode === "child" ? { parent_ids: [note.id] } : { memos: [note.id] },
                                    createdAt,
                                    updatedAt: createdAt
                                  };
                              ...
                              placeholder: memoInputMode === "child" ? "添加子卡..." : "写下批注...",
                              rows: 2
                            }
                          ) }),
                          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-input-toolbar", children: [
                            ...
                            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", alignItems: "center", gap: "8px" }, children: [
                              /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "mcard-inline-edit-cancel", onClick: () => {
                                setMemoInputNoteId(null);
                                setMemoInputText("");
                              }, children: "取消" }),
                              /* @__PURE__ */ jsxRuntimeExports.jsx(
                        ...
                        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-card-actions", onClick: (e) => e.stopPropagation(), children: [
                          /* @__PURE__ */ jsxRuntimeExports.jsxs(
                            "button",
                            {
                              className: "mcard-card-action-btn",
                              onClick: () => {
                                if (isAddingMemo && memoInputMode === "child") {
                                  setMemoInputNoteId(null);
                                  setMemoInputText("");
                                } else {
                                  setMemoInputMode("child");
                              ...
                              children: [
                                ...
                                childNotes.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "mcard-card-action-badge", children: childNotes.length })
                              ]
                            }
                          ),
                          /* @__PURE__ */ jsxRuntimeExports.jsxs(
                            "button",
                            {
                              className: "mcard-card-action-btn",
                              onClick: () => {
                                if (isAddingMemo && memoInputMode === "memo") {
                                  setMemoInputNoteId(null);
                                  setMemoInputText("");
                                } else {
                                  setMemoInputMode("memo");
                                  setMemoInputNoteId(note.id);
                                  setMemoInputText("");
                                  setTimeout(() => {
                                    const el = document.querySelector(`[data-memo-input="${note.id}"]`);
                              ...
                              title: "批注",
                              children: [
                                /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { d: "M22 17a2 2 0 0 1-2 2H6.828a2 2 0 0 0-1.414.586l-2.202 2.202A.71.71 0 0 1 2 21.286V5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2z" }) }),
                                memoNotes.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "mcard-card-action-badge", children: memoNotes.length })
                              ]
                            }
                          ),
                          /* @__PURE__ */ jsxRuntimeExports.jsx(
                            "button",
                            {
                              className: "mcard-card-action-btn mcard-card-action-open",
                              onClick: () => {
                                setDetailNavStack([]);
                                setDetailNote(note);
                              },
          ...
          tab === "settings" && /* @__PURE__ */ jsxRuntimeExports.jsx(
            SettingsView,
            {
              ...
              onInboxApiSync: handleInboxApiSync,
              inboxApiSyncing,
              inboxApiMessage,
        ...
        detailNote && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mcard-detail-overlay", onClick: () => {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-detail-header", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "mcard-detail-back", onClick: () => goBackFromDetail(), children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mcard-detail-section mcard-detail-parents", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-edit-relation-row", style: { borderTop: "none", paddingTop: 0 }, children: [
            ...
            detailNote.parent_ids && detailNote.parent_ids.length > 0 && detailNote.parent_ids.map((pid) => {
              ...
              const preview = getNotePreview(parentNote);
              return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-edit-relation mcard-edit-relation-parent", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "mcard-edit-relation-info", style: { cursor: "pointer" }, onClick: () => navigateToDetail(parentNote), children: preview }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "mcard-edit-relation-remove", onClick: (e) => {
                  var _a;
                  e.stopPropagation();
                  const updated = notes.map(
          ...
          detailEditing ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-detail-section mcard-detail-card-edit", style: { position: "relative" }, children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                className: "mcard-edit-title-input",
            ...
            showDetailTagPicker && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-tag-picker", style: { top: "80px", left: "10px" }, onClick: (e) => e.stopPropagation(), children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mcard-tag-picker-title", children: "选择标签" }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-tag-picker-list", children: [
                allTags.filter((t2) => !detailTagPickerFilter || t2.toLowerCase().includes(detailTagPickerFilter.toLowerCase())).slice(0, 5).map((tag, idx) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-tag-picker-item", onClick: () => {
                  const ta = document.querySelector(".mcard-detail-edit-area");
                  if (ta) {
                    const start = ta.selectionStart;
                    const val = ta.value;
                ...
                detailTagPickerFilter && !allTags.some((t2) => t2.toLowerCase() === detailTagPickerFilter.toLowerCase()) && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-tag-picker-item active", onClick: () => {
                  const ta = document.querySelector(".mcard-detail-edit-area");
                  if (ta) {
                    const start = ta.selectionStart;
                    const val = ta.value;
                    const newVal = val.substring(0, start) + `#${detailTagPickerFilter} ` + val.substring(ta.selectionEnd);
                    ta.value = newVal;
                    ta.selectionStart = ta.selectionEnd = start + detailTagPickerFilter.length + 2;
                    setDetailEditText(newVal);
                  }
                  setShowDetailTagPicker(false);
                  setDetailTagPickerFilter("");
          ...
          (() => {
            ...
            return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-detail-section mcard-detail-child-cards", children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `mcard-detail-child-cards-list ${childCardGridMode ? "mcard-child-grid" : ""}`, children: [
                filteredChildCards.map((childNote) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-detail-child-card", onClick: () => navigateToDetail(childNote), children: [
          ...
          (() => {
            const memoNotes = notes.filter((n) => n.memos && n.memos.includes(detailNote.id));
            if (memoNotes.length === 0) return null;
            return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-detail-section mcard-detail-child-cards", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mcard-detail-section-label-row", children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "mcard-detail-section-label", children: `批注 (${memoNotes.length})` }) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mcard-detail-child-cards-list", children: memoNotes.map((memoNote) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-detail-child-card", onClick: () => navigateToDetail(memoNote), children: [
                memoNote.title && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mcard-detail-child-card-title", children: memoNote.title }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mcard-detail-child-card-content", children: (() => {
                  const cacheKey = `${memoNote.id}::hide::${memoNote.content}`;
                  let html = noteHtmlCacheRef.current.get(cacheKey);
                  if (html === void 0) {
                    html = renderNoteHtml(memoNote.content, resolveAssetUrl, { hideTags: true, notes });
                    noteHtmlCacheRef.current.set(cacheKey, html);
                  }
                  return /* @__PURE__ */ jsxRuntimeExports.jsx("span", { dangerouslySetInnerHTML: { __html: html } });
                })() }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-detail-child-card-footer", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "mcard-detail-child-card-date", children: formatAgo(memoNote.createdAt) }),
                  memoNote.tags.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mcard-detail-child-card-tags", children: memoNote.tags.map((tag) => /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "mcard-detail-tag", children: [
                    "#",
                    tag
                  ] }, tag)) })
                ] })
          ...
          (() => {
            const backlinkNotes = findBacklinkNotes(notes, detailNote);
            if (backlinkNotes.length === 0) return null;
            return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-detail-section mcard-detail-child-cards", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mcard-detail-section-label-row", children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "mcard-detail-section-label", children: `反向引用 (${backlinkNotes.length})` }) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mcard-detail-child-cards-list", children: backlinkNotes.map((blNote) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-detail-child-card", onClick: () => navigateToDetail(blNote), children: [
                blNote.title && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mcard-detail-child-card-title", children: blNote.title }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mcard-detail-child-card-content", children: (() => {
                  const cacheKey = `${blNote.id}::hide::${blNote.content}`;
                  let html = noteHtmlCacheRef.current.get(cacheKey);
                  if (html === void 0) {
                    html = renderNoteHtml(blNote.content, resolveAssetUrl, { hideTags: true, notes });
                    noteHtmlCacheRef.current.set(cacheKey, html);
                  }
                  return /* @__PURE__ */ jsxRuntimeExports.jsx("span", { dangerouslySetInnerHTML: { __html: html } });
                })() }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-detail-child-card-footer", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "mcard-detail-child-card-date", children: formatAgo(blNote.createdAt) }),
                  blNote.tags.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mcard-detail-child-card-tags", children: blNote.tags.map((tag) => /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "mcard-detail-tag", children: [
                    "#",
                    tag
                  ] }, tag)) })
                ] })
        ...
        showSearchExprModal && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mcard-detail-overlay", onClick: () => {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-expr-bottom", style: { flex: 1, borderTop: "none" }, children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mcard-expr-main", style: { padding: "0 16px" }, children: activeExprId ? (() => {
              ...
              let result = [...notes];
              if (!settings.showMemoNotesInList) {
                result = result.filter((n) => !n.memos || n.memos.length === 0);
              }
              if (expr.conditions.length > 0) {
        ...
        toastMsg && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: {
          position: "absolute",
          bottom: 16,
          left: "50%",
          transform: "translateX(-50%)",
          ...
          whiteSpace: "nowrap",
          border: `1px solid ${morandi.color}33`
        }, children: toastMsg })
      ] })
    }
  );
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-settings-area", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-settings-content", children: [
      ...
      activeGroup === "inbox" && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-settings-section", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mcard-settings-subtitle", children: "API 配置" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mcard-settings-desc", children: "填写完整的 API 地址拉取 Inbox 数据" }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-flomo-form-row", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "mcard-flomo-form-label", children: "API URL" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                type: "text",
                className: "mcard-flomo-input",
                placeholder: "",
                value: inboxConfig.inboxApiUrl,
                onChange: (e) => onInboxConfigChange({ ...inboxConfig, inboxApiUrl: e.target.value })
              }
            )
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mcard-flomo-form-actions", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
            ...
            {
              className: "mcard-flomo-btn mcard-flomo-btn-primary",
              onClick: onInboxApiSync,
              disabled: inboxApiSyncing || !inboxConfig.inboxApiUrl,
              children: inboxApiSyncing ? "同步中..." : "开始同步"
            }
          ) }),
          inboxApiMessage && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mcard-flomo-message", children: inboxApiMessage })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mcard-settings-section", children: [
